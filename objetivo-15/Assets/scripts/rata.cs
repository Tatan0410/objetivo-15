...
public class rata : MonoBehaviour
{
    ...
    void SoltarPlasticos()
    {
        ...
        Debug.Log($"[Rata] SoltarPlasticos en '{escena}' | drop={probabilidadDrop} cantidad={cantidadDrop} prefabs={(prefabsPlasticos!=null?prefabsPlasticos.Length:0)} pos={transform.position}");

        if (Random.value > probabilidadDrop)
        {
            Debug.Log($"[Rata] Drop falló por probabilidad ({probabilidadDrop}) en '{escena}'");
            return;
        }
        ...
        {
            Debug.LogWarning($"[Rata] prefabsPlasticos vacío en '{escena}' ({gameObject.name}) - usando fallback Resources");
        ...
        if (validos == 0)
        {
            Debug.LogWarning($"[Rata] Ningún prefab válido para drop en '{escena}' ({gameObject.name})");
            return;
        }
        ...
        Debug.Log($"[Rata] Spawneando {validos} plasticos en '{escena}' pos={posSpawn} overlap={Physics2D.OverlapCircle(posSpawn,0.2f) != null}");
